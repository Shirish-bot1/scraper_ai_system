    ...
    with open(csv_file_path, mode="r", encoding="utf-8-sig") as file:
        ...
        for row in reader:
            try:
                ...
                if not mun_name:
                    print("⚠️ Skipping empty municipality row")
                    continue
                ...
                mun = (
                    ...
                    .filter(Municipality.municipality_name == mun_name)
                    .first()
                )

                if not mun:
                    print(f"⚠️ NOT FOUND in DB: {mun_name}")
                    continue
                ...
                exists = (
                    ...
                    .filter(MunicipalityOfficial.municipality_id == mun.id)
                    .first()
                )

                if exists:
                    print(f"🔁 Already exists: {mun_name}")
                    continue
...
if __name__ == "__main__":
    import_officials_csv("municipality_official.csv")